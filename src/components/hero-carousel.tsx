        ...
        {heroItems.map((item) => (
          <CarouselItem key={item.id}>
            <div className="relative aspect-[16/7] md:aspect-[16/6] lg:aspect-[16/5] w-full">
              <Image src={item.image || "/placeholder.svg"} alt={item.title} fill priority className="object-cover" />
              <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-center p-4">
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4 drop-shadow-md">
                  {item.title}
                </h2>
                <p className="text-lg sm:text-xl text-gray-200 mb-6 max-w-2xl drop-shadow-sm">{item.description}</p>
                <Link href={item.link}>
                  <Button
                    size="lg"
